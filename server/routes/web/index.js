...
module.exports = app => {
    ...
    router.get('/heroes/init', async (req, res) => {
        ...
        for(let cat of heroesJSON) {
            ...
            cat.heroes.map( hero => {
                let filters = newHeroes.filter(el => el.name === hero.name)
                if (filters.length > 0) {
                    filters[0].categories.push(category)
                } else {
                    hero.categories = [category]
                    newHeroes.push(hero)
                }
    ...
    router.get('/hero/list', async (req, res) => {
        ...
        // 首位插入热门数据
        const subCats = cats.map(v => v._id)
        const hotData = await Hero.aggregate([
            {
                $match: {
                    categories: { $in: subCats }
                }
